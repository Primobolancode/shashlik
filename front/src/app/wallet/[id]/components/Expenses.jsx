...
export default function Expenses({wallet, setLoading, fetchData}) {
    ...
    const deleteExpense = async (expense_id) => {
        ...
        try {
            const response = await fetch(`http://api.localhost/api/v1/event/${id}/expense/${expense_id}`, requestOptions)
    ...
    const addExpense = async () => {
        ...
        try {
            const response = await fetch(`http://api.localhost/api/v1/event/${id}/expense`, requestOptions)
            const data = await response.json();
            await fetchData()
            setCreditor('')
            setDebtors([])
